...
gameState.cursors = this.input.keyboard.createCursorKeys();
    ...
    this.add.text(100, 50, 'Click to play again!', { fontFamily: 'Arial', fontSize: 36, color: '#00000' })
  ...
  update() {
   ...
   const restart = ()=> {
      this.cameras.main.fade(800, 0, 0, 0, false, function(camera, progress) {
        // gameState.player.anims.play('sled', false);
        if (progress > .9) {
          // this.scene.stop(this.levelKey);
          // this.scene.start(this.nextLevel[this.levelKey]);
          this.scene.stop(this.levelKey);
...
//             if (/*gameState.cursors.up.isDown*/this.input.mouse('pointerDown') ) {
// restart();
//     }
